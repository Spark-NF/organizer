...
LoaderCondition::LoaderCondition(QString key, std::shared_ptr<Loader> loader, std::shared_ptr<Comparator> comparator)
	: m_key(std::move(key)), m_loader(std::move(loader)), m_comparator(std::move(comparator))
{}

bool LoaderCondition::match(Media &media) const
{
