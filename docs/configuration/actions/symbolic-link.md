...
* **dest**: where to create the symbolic link (absolute, or relative to the source file's directory). Supports `{key}` placeholders (see [Loader](../conditions/loader.md)) resolved at execution time.
